...
// Keep samples practical so the diff has signal.
...
struct DSU{ vector<int> p, r;
  ...
  bool unite(int a,int b){a=f(a);b=f(b); if(a==b) return false;
    if(r[a]<r[b]) swap(a,b); p[b]=a; if(r[a]==r[b]) r[a]++; return true; }
};
int main(){ ios::sync_with_stdio(false); cin.tie(nullptr);
  DSU d(5); d.unite(0,1); d.unite(3,4); cout<<(d.f(1)==d.f(0)); }`,
...
fun main() {
    val q = Query().where("age", ">", 20).where("name", "LIKE", "'A%'")
    println(q)
}`,
...
.card { padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; }`
};
